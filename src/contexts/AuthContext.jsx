...
export function AuthProvider({ children }) {
  ...
  const [demoRestId, setDemoRestId]         = useState(() => {
    try {
      return localStorage.getItem('demo_rest_id')
    } catch { return null }
  })
  const [demoResolving, setDemoResolving]   = useState(false)
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_, session) => {
      ...
      // If we see the demo user, instantly lock loading (unless cached)
      if (session?.user?.email === 'demo@example.com') {
        if (!localStorage.getItem('demo_rest_id')) {
          setDemoResolving(true)
        }
      }
    })
  ...
  // Resolve demo restaurant ID fallback (with caching)
  useEffect(() => {
    if (user?.email === 'demo@example.com') {
      // If we already have a cached ID, skip resolving
      if (demoRestId) return
      ...
      supabase.from('restaurants')
        ...
        .then(({ data }) => { 
          if (data) {
            console.log('[Auth] Resolved and cached demoRestId:', data.id)
            setDemoRestId(data.id) 
            localStorage.setItem('demo_rest_id', data.id)
          } else {
            console.warn('[Auth] Could not find demo-cafe in restaurants table!')
          }
        })
        .finally(() => setDemoResolving(false))
    } else {
      setDemoRestId(null)
      setDemoResolving(false)
    }
  }, [user, demoRestId])
  ...
  const isDemoGuest   = user?.email === 'demo@example.com'
  const role          = (impersonating || isDemoGuest) ? 'admin' : realRole
  const restaurantId  = isDemoGuest ? demoRestId : (impersonating ? impersonating.restaurant_id : realRestId)
  ...
  const handleDemoLogin = useCallback(async (targetRole = 'admin') => {
    try {
  ...
  const value = {
    ...
    isSuperAdmin, isAdmin, isWaiter, isDemoGuest,
    impersonating, startImpersonation, stopImpersonation,
    signOut, handleDemoLogin, kicked, dismissKicked,
    loading: user === undefined || demoResolving,
  }

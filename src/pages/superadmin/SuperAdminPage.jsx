...
function AddUserInlineForm({ restaurantId, onUserCreated, onCancel, showToast }) {
  ...
  const handleCreate = async (e) => {
    e.preventDefault()
    if (!email || !pass || emailStatus === 'taken') return
    setSaving(true)
    try {
      await createUser({
        email: email.trim(),
  ...
  return (
    <div className="bg-raised p-4 rounded-xl border border-amber/20 animate-fade-in mt-2 mb-4">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-[10px] uppercase font-bold text-amber tracking-widest">
          ➕ New User for this Restaurant
        </h4>
        <button onClick={onCancel} className="text-faint hover:text-danger text-xs">✕</button>
      </div>
          ...
          <div>
            ...
            <div className="relative">
              <input 
                ...
                placeholder="staff@restaurant.com" className="input text-xs" required 
              />
              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                {emailStatus === 'checking' && <MiniSpinner />}
                {emailStatus === 'available' && <span className="text-done text-xs">✅</span>}
                {emailStatus === 'taken' && <span className="text-danger text-xs">❌</span>}
              </div>
            </div>
          </div>
          <div>
            <label className="label text-[10px]">Initial Password</label>
            <input 
              type="password" value={pass} onChange={e => setPass(e.target.value)}
              placeholder="Min 6 chars" className="input text-xs" minLength={6} required 
            />
          </div>
        </div>
        <div className="flex items-center justify-between gap-4">
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer group">
              <input 
                type="radio" name="role" value="waiter" checked={role === 'waiter'} 
                onChange={e => setRole(e.target.value)} className="hidden"
              />
              <div className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center transition-all
                              ${role === 'waiter' ? 'border-amber bg-amber shadow-[0_0_8px_rgba(255,191,0,0.4)]' : 'border-border'}`}>
                {role === 'waiter' && <div className="w-1.5 h-1.5 rounded-full bg-base" />}
              </div>
              <span className={`text-[11px] font-semibold tracking-wide transition-colors ${role === 'waiter' ? 'text-amber' : 'text-faint'}`}>
                WAITER
              </span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer group">
              ...
              />
              <div className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center transition-all
                              ${role === 'admin' ? 'border-amber bg-amber shadow-[0_0_8px_rgba(255,191,0,0.4)]' : 'border-border'}`}>
                {role === 'admin' && <div className="w-1.5 h-1.5 rounded-full bg-base" />}
              </div>
              <span className={`text-[11px] font-semibold tracking-wide transition-colors ${role === 'admin' ? 'text-amber' : 'text-faint'}`}>
                ADMIN
              </span>
            </label>
          </div>
          <button 
            type="submit" disabled={saving || emailStatus === 'taken'} 
            className="btn-amber text-[10px] py-1.5 px-6"
          >
            {saving ? <MiniSpinner /> : '🚀 Create User'}
          </button>
        </div>
      </form>
    </div>
      ...
      {expanded && (
        <div className="border-t border-border px-4 pb-4 space-y-4 animate-fade-in">
          ...
          <div className="flex items-center justify-between gap-2 pt-2">
            <p className="text-mid text-xs font-semibold uppercase tracking-wider">Staff Management</p>
            <button 
              onClick={() => setShowAddUser(!showAddUser)}
              className={`text-[10px] font-bold px-2.5 py-1 rounded-lg border transition-all
                          ${showAddUser ? 'bg-danger/10 border-danger/30 text-danger' : 'bg-amber/10 border-amber/30 text-amber'}`}
            >

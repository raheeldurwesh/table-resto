...
export default function LandingPage() {
  ...
  const handleDemoClick = async (targetRole = 'admin') => {
    const res = await handleDemoLogin(targetRole)
    if (res.success) {
      navigate(targetRole === 'admin' ? '/demo-cafe/admin' : '/demo-cafe/waiter')
    } else {
  ...
  const ROLES = [
    ...
    { id: 'demo',       name: 'Live Demo',         icon: '🌟', desc: 'Explore the Sandbox',   accent: 'amber', isDemo: true },
  ]
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 animate-slide-up [animation-delay:400ms]">
            ...
            <button
              ...
              className="px-10 py-4 rounded-2xl border-2 border-amber/40 text-amber font-display font-bold 
                         hover:bg-amber/10 transition-all backdrop-blur-sm"
            >
              Try Free Demo
            </button>
          </div>
        ...
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {ROLES.map((r, i) => (
              <div
                key={r.id}
                onClick={() => r.isDemo ? handleDemoClick('admin') : navigate(`/login/${r.id}`)}
                className={`group glass-card p-6 sm:p-10 rounded-3xl cursor-pointer hover:border-amber/40 transition-all active:scale-[0.98] animate-scale-in
                           ${r.isDemo ? 'border-amber/30 bg-amber/5' : ''}`}
                style={{ animationDelay: `${i * 100}ms` }}
              >
                <div className={`w-20 h-20 rounded-2xl bg-amber/10 flex items-center justify-center text-4xl mb-6 group-hover:scale-110 transition-transform ${r.isDemo ? 'animate-pulse' : ''}`}>
                  {r.icon}
                </div>
                ...
                <div className="flex items-center justify-center gap-2 text-amber font-bold text-sm uppercase tracking-widest">
                  {r.isDemo ? 'Explore' : 'Sign In'} <span>→</span>
                </div>
              </div>
            ))}
          </div>

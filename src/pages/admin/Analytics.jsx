    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            onClick={() => {
              if (isDemoGuest) return showToast('⚠️ Reporting restricted in Demo Mode')
              generateReport({ type: 'daily', orders: todayOrders, label: dateLabel, config })
            }}
          ...
          <button
            onClick={() => {
              if (isDemoGuest) return showToast('⚠️ Reporting restricted in Demo Mode')
              generateReport({ type: 'monthly', orders: monthOrders, label: monLabel, config })
            }}
            className="btn-ghost text-sm"

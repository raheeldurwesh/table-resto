...
export default function MenuManager({ restaurantId }) {
  ...
  const handleSave = async (form, imgFile) => {
    if (isDemoGuest) {
      showToast('⚠️ Action restricted in Demo Mode')
      return
    }
    setSaving(true)
    try {
  ...
  const handleDelete = async (item) => {
    if (isDemoGuest) {
      showToast('⚠️ Deletion restricted in Demo Mode')
      return
    }
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={() => {
              if (isDemoGuest) return showToast('⚠️ Import restricted in Demo Mode')
              setShowImport(true)
            }} 
            ...
            <div key={item.id} className="card overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (isDemoGuest) return showToast('⚠️ Restricted in Demo Mode')
                      toggleAvailability(item.id, item.available !== false)
                    }}
